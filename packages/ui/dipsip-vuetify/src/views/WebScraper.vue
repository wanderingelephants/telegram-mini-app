  ...
  <v-card class="ma-4">
    ...
    <v-card-text>
      ...
      <v-expansion-panels class="mt-4">
        <v-expansion-panel
          v-for="(category, index) in processedResults"
          :key="index"
        >
...
export default {
  ...
  methods: {
    ...
    async get_funds_for_category_url(
      category,
      mf_category_url,
      inputSchemeCodes
    ) {
      if (category === "etfs") {
        let funds =
          inputSchemeCodes.length === 0
            ? this.etfFunds
            : this.etfFunds.filter(
                (e) =>
                  e &&
                  e.schemeCode &&
                  inputSchemeCodes.indexOf(e.schemeCode) > -1
              );
      ...
      if (headerRow) {
        ...
        columnIndices.name = headers.findIndex((h) =>
          h.includes("Scheme Name")
        );
        columnIndices.plan = headers.findIndex((h) => h.includes("Plan"));
        columnIndices.category = headers.findIndex((h) =>
          h.includes("Category Name")
        );
        columnIndices.rating = headers.findIndex((h) =>
          h.includes("Crisil Rating")
        );
      ...
      rows.forEach((row) => {
        ...
        if (cells.length >= 1) {
          ...
          if (link) {
            ...
            let schemeCode;
            let urlCategory;
            if (category !== "index-fundsetfs") {
              //console.log("Process non index-fundsetfs")
              url = link.href;
              //console.log("url", url);
              // Skip ad clicks and non-mutual fund URLs
              if (
                url.includes("Click_Tracker") ||
              ...
              if (
                name.toLowerCase().includes("invest now") ||
              ...
              schemeCode = url.split("/").pop();
              console.log("schemeCode", schemeCode);
              if (
                !schemeCode ||
                schemeCode.includes("?") ||
                schemeCode === "MC_Click_Tracker"
              ) {
                return;
              }
              ...
              urlCategory = urlParts.length >= 5 ? urlParts[4] : "";
              if (
                !urlCategory ||
                urlCategory.includes("?") ||
                urlCategory.includes("clk")
              ) {
                return;
              }
            ...
            if (
              ...
              funds.push({
                ...
                urlCategory,
                plan:
                  columnIndices.plan !== -1
                    ? cells[columnIndices.plan]?.textContent.trim()
                    : "",
                category:
                  columnIndices.category !== -1
                    ? cells[columnIndices.category]?.textContent.trim()
                    : "",
                rating:
                  columnIndices.rating !== -1
                    ? cells[columnIndices.rating]?.textContent.trim()
                    : "",
                aum:
                  columnIndices.aum !== -1
                    ? cells[columnIndices.aum]?.textContent.trim()
                    : "",
                returns,
              });
            } else {
    ...
    async processFundDetails(fund) {
      ...
      const expensesRatioElement = doc.querySelector(
      ...
      const categoryAverageElement = doc.querySelector(
        "#mc_content > div > section.clearfix.section_one > div > div.common_left > div:nth-child(3) > div.right_section > div.top_section > table > tbody > tr:nth-child(1) > td:nth-child(2) > div.grayvalue"
      );

      fund.expensesRatio = expensesRatioElement?.textContent.trim() || "N/A";
      fund.expensesRatioCategoryAverage =
        categoryAverageElement?.textContent.trim() || "N/A";

      // Get portfolio URL and fetch holdings
      const baseUrl = fund.url.replace("-growth", "").replace("/nav/", "/");
      const lastSlashIndex = baseUrl.lastIndexOf("/");
      const portfolioUrl =
        baseUrl.slice(0, lastSlashIndex) +
        "/portfolio-holdings" +
        baseUrl.slice(lastSlashIndex);
      ...
      let holdingsDateText =
        holdingsDateSpan == null
          ? null
          : this.parseHoldingsDate(holdingsDateSpan);
    ...
    async processCategory(category, mf_scheme_code) {
      ...
      mf_scheme_code === "" || mf_scheme_code === "All"
        ...
        : (funds = await this.get_funds_for_category_url(
            category,
            categoryUrl,
            mf_scheme_code.split(",").map((item) => item.trim())
          ));
