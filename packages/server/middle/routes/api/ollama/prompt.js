...
const route = async (req, res) => {
  try{
    const {model, promptTemplate, query, operation = 'savePrompt'} = req.body
    ...
    if (operation === 'savePrompt') {
      ...
      const outputFile = PROMPTS_FOLDER + '/' + 'mutual_funds_prompt.txt'
      fs.writeFileSync(outputFile, promptTemplate)
      res.status(200).json('Saved')
      return;  
    }
    else {
      const response = await sendPrompt(model , 'mutual_funds_prompt.txt', query)
