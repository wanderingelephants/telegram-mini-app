...
function persistUser(req) {
    ...
    const { id, first_name, last_name, username } = req.query
    let expiry_date = new Date(); expiry_date.setHours(0, 0, 0, 0);
    expiry_date = new Date(expiry_date.getTime() + 30 * 24 * 3600 * 1000)
    try {
        const insert_resp = db.prepare('insert into user_profile(tg_id, tg_firstname, tg_lastname, tg_username, expiry_date) values (@tg_id, @tg_firstname, @tg_lastname, @tg_username, @expiry_date)').run({ tg_id: id, tg_firstname: first_name, tg_lastname: last_name, tg_username: username, expiry_date: expiry_date.getTime() })
...
let route = async (req, res) => {
    try {
        ...
        if (verifyTelegramAuth(req.query)) {
            ...
            const userRecord = persistUser(req)
            const db_path = process.env.SQLITE_DB + '/dipsip.db'
            const options = { fileMustExist: true }
            const db = require('better-sqlite3')(db_path, options)
            db.pragma('journal_mode = WAL')
            const configRecord = db.prepare('select base_amt, trigger, buy_factor, instrument from user_config where user_id=@user_id').get({ user_id: userRecord.id })
